g.impute = function(M, I, params_cleaning = c(), desiredtz = "",
                    ...
                    TimeSegments2Exclude = c(), acc.metric = "ENMO", 
                    ID, qwindowImp = c(), ...) {
  ...
  # CATigraphy extension: add cursor-selected windows to protocol data removal.
  if (length(TimeSegments2Exclude) > 0) {
    timelinePOSIX = iso8601chartime2POSIX(metashort$timestamp, tz = desiredtz)
    for (i in 1:nrow(TimeSegments2Exclude)) {
      selectedShort = which(
        timelinePOSIX >= TimeSegments2Exclude$windowstart[i] &
          timelinePOSIX <= TimeSegments2Exclude$windowend[i]
      )
      if (length(selectedShort) > 0) {
        selectedMedium = unique(ceiling(selectedShort / n_short_in_mediumEpoch))
        selectedMedium = selectedMedium[selectedMedium <= length(r4)]
        r4[selectedMedium] = 1
      }
    }
  }
